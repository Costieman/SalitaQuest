...
const profileRuntimeFile = "src/core/learner-profile-runtime-v1.js";
const profileRuntimeSource = read(profileRuntimeFile);
...
for (const [file,source] of [[rootFile,rootSource],[profileRuntimeFile,profileRuntimeSource],[adapterFile,adapterSource],[featureFile,featureSource],["src/config/course-manifest.js",manifestSource]]) new vm.Script(source,{filename:file});
...
for (const marker of [
  ...
  "SalitaQuestLearnerProfileRuntimeV1","function catalogueFeatureReady","function sessionValue()","function save()","function badgeArtValue(id)","function bossReadyValue()",
...
for (const [id,course] of Object.entries(manifestContext.window.SalitaQuestCourseManifest.courses)) {
  const profiles=course.scripts.indexOf("src/core/learner-profile-runtime-v1.js?v=5.6.1");
  ...
  if (!(profiles>=0 && reward>profiles && adapter>reward && feature>adapter && chest>feature)) fail(`${id} badge runtime order changed`);
...
vm.createContext(context);vm.runInContext(profileRuntimeSource,context,{filename:profileRuntimeFile});vm.runInContext(adapterSource,context,{filename:adapterFile});vm.runInContext(featureSource,context,{filename:featureFile});
...
if (writes.length!==3||!writes[0].includes(profileRuntimeFile)||!writes[1].includes(adapterFile)||!writes[2].includes(featureFile)) fail("Historical parser-time dependency order changed");
