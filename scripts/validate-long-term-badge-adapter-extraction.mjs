...
for (const [name,source] of [["root",root],["profiles",profiles],["adapter",adapter],["feature",feature],["loader",loader],["worker",worker]]) new vm.Script(source,{filename:name});
...
if ((profiles.match(/localStorage\.getItem/g)||[]).length !== 1 || (profiles.match(/sessionStorage\.getItem/g)||[]).length !== 1) fail("Shared learner profile read ownership changed");
if (/localStorage|sessionStorage/.test(adapter)) fail("Badge adapter still owns learner storage");
if (!adapter.includes("SalitaQuestLearnerProfileRuntimeV1")) fail("Badge adapter does not use the shared learner profile runtime");
if (/localStorage\.(?:setItem|removeItem)|sessionStorage\.(?:setItem|removeItem)/.test(adapter)) fail("Badge adapter must not mutate storage directly");
...
if (writes.length !== 3 || !writes[0].includes("learner-profile-runtime-v1.js?v=5.6.1") || !writes[1].includes("badge-catalogue-runtime-v1.js?v=5.6.0") || !writes[2].includes("src/features/badges/long-term-badges-v1.js?v=5.6.0")) fail("Parser-time root compatibility order changed");
