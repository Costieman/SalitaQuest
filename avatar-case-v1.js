(() => {
  ...
  function cleanIds(values, profile = activeRecord().profile) {
    ...
    for (const raw of Array.isArray(values) ? values : []) {
      ...
      result.push(item.id);
      if (result.length >= MAX_CASE_AVATARS) break;
    }
    return result;
  }
  ...
  function getIds() {
    ...
    return cleanIds(profile.avatarCaseIds || legacy || [], profile);
  }

  function getAvatars() {
    return getIds().map(id => model()?.get?.(id)).filter(Boolean);
  }

  function persist(ids, options = {}) {
    const {store, profile} = activeRecord();
    if (!profile) return [];
    const cleaned = cleanIds(ids, profile);
  ...
  function slotMarkup(item, index, total) {
    if (!item) {
      return `<button class="sq-avatar-case-slot is-empty" type="button" data-avatar-case-open-picker aria-label="Choose an avatar for empty case slot ${index + 1}">
        <span class="sq-avatar-case-empty-mark">＋</span><strong>Empty slot</strong><small>Choose an unlocked avatar</small>
      </button>`;
    }
    return `<article class="sq-avatar-case-slot is-filled" data-avatar-case-id="${item.id}">
      ...
      <div class="sq-avatar-case-controls" aria-label="Reorder ${esc(item.name)}">
        <button type="button" data-avatar-case-move="-1" data-avatar-case-id="${item.id}" ${index === 0 ? "disabled" : ""} aria-label="Move ${esc(item.name)} left">←</button>
        <button type="button" data-avatar-case-move="1" data-avatar-case-id="${item.id}" ${index === total - 1 ? "disabled" : ""} aria-label="Move ${esc(item.name)} right">→</button>
        <button type="button" data-avatar-case-remove="${item.id}" aria-label="Remove ${esc(item.name)} from Avatar Case">×</button>
      </div>
    </article>`;
  }
  ...
  function ensurePicker() {
    ...
    picker.innerHTML = `
      ...
      <section class="sq-avatar-case-picker-dialog" role="dialog" aria-modal="true" aria-labelledby="sqAvatarCasePickerTitle">
        <header><div><p>AVATAR CASE</p><h3 id="sqAvatarCasePickerTitle">Choose up to four favourites</h3><span>Only avatars already unlocked on this profile are available.</span></div><button type="button" data-avatar-case-picker-close aria-label="Close Avatar Case picker">×</button></header>
    ...
    picker.addEventListener("click",event => {
      ...
      if (choice) toggleDraft(choice.dataset.avatarCaseChoice);
  ...
  function renderPicker() {
    ...
    const {profile} = activeRecord();
    const owned = ownedIds(profile).map(id => model()?.get?.(id)).filter(Boolean);
    const grid = modal.querySelector(".sq-avatar-case-picker-grid");
    const status = modal.querySelector(".sq-avatar-case-picker-status");
    status.textContent = `${pickerDraft.length} of ${MAX_CASE_AVATARS} selected`;
    ...
    window.SalitaAvatarArtwork?.repair?.(grid);
  }
  ...
  function toggleDraft(id) {
    ...
    if (pickerDraft.includes(item.id)) pickerDraft = pickerDraft.filter(value => value !== item.id);
      ...
      if (status) status.textContent = `The Avatar Case holds exactly ${MAX_CASE_AVATARS} favourites. Remove one before adding another.`;
      return;
    }
    renderPicker();
  }
  ...
  function handleDocumentClick(event) {
    ...
    if (moveButton && !moveButton.disabled) {
      ...
      move(moveButton.dataset.avatarCaseId,Number(moveButton.dataset.avatarCaseMove));
      return;
    }
    const removeButton = event.target.closest("[data-avatar-case-remove]");
    if (removeButton) { event.preventDefault(); remove(removeButton.dataset.avatarCaseRemove); }
  }
  ...
  if (model()) install();
  else document.addEventListener("salita:avatar-progression-ready",install,{once:true});
})();
