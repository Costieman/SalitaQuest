(() => {
  ...
  const profileProgressKey = `${PROFILE_PROGRESS_PREFIX}${activeId}`;
  let lastProgress = localStorage.getItem(BASE_PROGRESS);

  function syncProgress() {
    const progress = localStorage.getItem(BASE_PROGRESS);
    if (progress === lastProgress) return;
    if (progress) localStorage.setItem(profileProgressKey, progress);
    else localStorage.removeItem(profileProgressKey);
  ...
  function installProfileControl() {
    ...
    style.textContent = `
      ...
      .sq-profile-menu{position:absolute;right:0;bottom:60px;width:210px;padding:10px;background:#fff;border:1px solid #dbe8e4;border-radius:16px;box-shadow:0 18px 45px rgba(20,62,57,.22)}
      ...
      .sq-profile-identity small,.sq-profile-identity strong{display:block}.sq-profile-identity small{font-size:9px;letter-spacing:.12em;color:#607773;font-weight:800}.sq-profile-identity strong{font-size:14px;color:#173a37;max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .sq-profile-action{width:100%;border:0;border-radius:10px;padding:10px 11px;text-align:left;background:transparent;color:#173a37;font-weight:750;cursor:pointer}.sq-profile-action:hover{background:#edf7f3}.sq-profile-action.danger{color:#9b3434}
    ...
    control.innerHTML = `
      ...
      <div class="sq-profile-menu" hidden>
        <div class="sq-profile-identity">
          <img src="${avatarPath()}" alt="">
          <div><small>LEARNING AS</small><strong></strong></div>
        </div>
    ...
    if (version) version.textContent = "Version 5.4 Full-Screen Profiles";
  }
