...
function decodePngDataUrl(value, fieldName, expectedWidth, expectedHeight) {
  ...
  if (width !== expectedWidth || height !== expectedHeight) throw new Error(`${fieldName} must be ${expectedWidth}×${expectedHeight}px.`);
  return buffer;
}
...
async function serveMedia(req,res,headOnly=false) {
...
app.head("/media/:id/:variant.png",(req,res) => serveMedia(req,res,true));
app.get("/media/:id/:variant.png",(req,res) => serveMedia(req,res,false));
