...
function allowUpload(req) {
  ...
  const now = Date.now();
  const recent = (uploadWindows.get(key) || []).filter(timestamp => timestamp >= now - 60 * 60 * 1000);
...
app.post("/api/share-cards",async (req,res) => {
  ...
  try {
    ...
    const metadata = {
      ...
      title,
      description,
      learnerName:cleanText(req.body.learnerName,40),
      course:cleanText(req.body.course,40),
      campaign,
...
app.get("/media/:id/:variant.png",async (req,res) => {
  ...
  try {
    const [buffer] = await bucket.file(`images/${id}-${variant}.png`).download();
    const baseHeaders = {
    ...
    if (rangeHeader) {
      ...
      if (!range) return res.status(416).set("Content-Range",`bytes */${buffer.length}`).end();
...
app.get("/share/:id",async (req,res) => {
  ...
  try {
    ...
    res.set({
      ...
      "X-Robots-Tag":"all",
      "Link":`<${image}>; rel=preload; as=image`,
