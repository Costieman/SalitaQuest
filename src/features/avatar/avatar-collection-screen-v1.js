(() => {
  ...
  const current = () => {
    const currentModel = window.SalitaAvatarModel;
    const runtime = window.SalitaAvatarCollectionProfileRuntimeV1;
    return currentModel && runtime ? {model:currentModel, runtime} : null;
  };
  const ready = current();
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (value) {
        ...
        window.clearInterval(timer);
        reject(new Error("Avatar collection runtime was not available."));
      }
    }, 80);
  });
}
...
function refreshProfile() {
  const context = window.SalitaAvatarCollectionProfileRuntimeV1?.readContext?.();
  ...
  const result = window.SalitaAvatarCollectionProfileRuntimeV1?.equip?.(id);
  if (!result) return false;
  const {context, item} = result;
  model = context.model;
  collection = context.collection;
  ...
  waitForRuntime().then(({model:value}) => {
    model = value;
    buildScreen();
    installLauncher();
