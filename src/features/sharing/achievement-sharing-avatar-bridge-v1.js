(() => {
  ...
  function activeContext() {
    try {
      return profileRuntime()?.peekContext?.() || null;
    } catch {
      return null;
    }
  }
  ...
  function equippedAvatar() {
    ...
    const id = context?.collection?.equippedAvatarId || context?.profile?.avatarId || "anahaw";
  ...
  function ownsAvatar(id) {
    ...
    const context = activeContext();
    if (!item || !context?.profile || !context?.collection) return false;
    try {
      const owned = new Set(context.collection.ownedAvatarIds || []);
      if (context.profile.avatarId) owned.add(context.profile.avatarId);
