(() => {
  ...
  function runtime(){
    const api=window[RUNTIME_GLOBAL];
    if(!api) throw new Error("Social connections runtime adapter is unavailable.");
    return api;
  }
  function activeProfile(){ return runtime().activeProfile(); }
  function developerMode(){ return /(?:^|[?&])socialDebug=1(?:&|$)/.test(location.search) || location.hostname === "localhost" || location.hostname === "127.0.0.1"; }
  function apiBase(){
    const explicit=runtime().explicitApiBase();
    const developerOverride=developerMode()?runtime().developerApiBase():"";
  ...
  function developerMarkup(){
    ...
    const stored=runtime().developerApiBase();
  ...
  function openBadges(){
    try{ runtime().openView("badges"); }catch{}
  ...
  function install(){
    ...
    if(!window[RUNTIME_GLOBAL]||!document.getElementById("settingsView")){retry();return;}
    ...
    runtime().wrapView(baseSwitch => function switchViewWithSocialConnections(view){const result=baseSwitch.apply(this,arguments);if(view==="settings")window.setTimeout(refresh,30);return result;});
    window.SalitaQuestSocialConnections=Object.freeze({
      ...
      openSettings(){try{runtime().openView("settings");}catch{}window.setTimeout(()=>document.getElementById("socialLinksCard")?.scrollIntoView({behavior:"smooth",block:"center"}),80);}
    });
    render(); refresh();
  }
