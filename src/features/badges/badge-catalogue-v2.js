(() => {
  ...
  const API = "SalitaBadgeCatalogueFeatureV2";
  const INSTALL_FLAG = "__salitaQuestBadgeCatalogueV2Installed";
  const RELEASE = "5.4.23";
  const MAX_PENDING = 40;
  if (window[API]) { window[API].install?.(); return; }
  ...
  const countItemsAt = threshold => Object.values(appState()?.itemState || {}).filter(item => Number(item?.mastery || 0) >= threshold).length;
  const durableItemsAt = threshold => Object.values(appState()?.itemState || {}).filter(item => Number(item?.longTermMastery || item?.durableMastery || 0) >= threshold).length;
  const points = () => Number(runtime?.learningPoints?.() || 0);
  const level = () => Number(runtime?.level?.() || 1);
  ...
  const data = snapshot.badgeProgress || (snapshot.badgeProgress = {});
  ...
  snapshot.badgeMetrics = snapshot.badgeMetrics && typeof snapshot.badgeMetrics === "object" ? snapshot.badgeMetrics : {};
  snapshot.badgeMetrics.quickReviewItems = Number(snapshot.badgeMetrics.quickReviewItems || 0);
  snapshot.badgeMetrics.dailySessions = Number(snapshot.badgeMetrics.dailySessions || 0);
  return data;
}
  function existingIds() { return new Set(catalogue().map(badge => badge.id)); }
  ...
  function enhanceBadges() {
    ...
    const defaults = {
      ...
      boss_one:{category:"Conversation",unlockTest:()=>runtime.bossReady?.() || false}
    };
    list.forEach(badge => Object.assign(badge, defaults[badge.id] || {}, {image:badge.image || `badges/${badge.id}.png`}));
    const ids = existingIds();
    ADDITIONAL_BADGES.forEach(badge => { if (!ids.has(badge.id)) list.push({...badge,image:`badges/${badge.id}.png`}); });
  }

  function isEarned(badge) { try { return Boolean(badge.test?.(appState())); } catch { return false; } }
  function isAvailable(badge) { try { return badge.unlockTest ? Boolean(badge.unlockTest(appState())) : true; } catch { return false; } }

  function progressFor(badge) {
    if (!badge.target || typeof badge.current !== "function") return null;
    let current = 0;
    try { current = Math.max(0, Number(badge.current(appState()) || 0)); } catch {}
  ...
  function badgeFallback(badge) {
    try { return runtime.badgeArt?.(badge.id) || `<span>${badge.icon || "🏅"}</span>`; }
  ...
  function renderCatalogue() {
    ...
    const data = badgeState();
    const list = catalogue();
    const ranked = list.map((badge, index) => {
    ...
    if (summary) summary.textContent = `${count} of ${list.length} earned`;
    window.dispatchEvent(new CustomEvent("salita:badges-rendered", {detail:{earned:count,total:list.length}}));
  }
  ...
  async function animateBadge(badge) {
    ...
    const reduced = Boolean(appState()?.settings?.reducedMotion) || matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduced || !target) {
  ...
  async function playPending() {
    ...
    if (!pending) return;
    const badge = catalogue().find(item=>item.id===pending.id);
  ...
  runtime = candidate;
  if (!runtime?.catalogueFeatureReady?.()) { retry(); return false; }
  ...
  runtime.wrapFunction("recordDailyAnswer", baseRecordDailyAnswer => function recordDailyAnswerWithBadgeMetrics(correct,isReview=false) {
    const result = baseRecordDailyAnswer.apply(this,arguments);
    const state = appState();
    if (runtime.session()?.mode === "quick") state.badgeMetrics.quickReviewItems += 1;
    syncEarned();
    runtime.save();
    return result;
  });

  runtime.wrapFunction("recordDailySession", baseRecordDailySession => function recordDailySessionWithBadgeMetrics() {
    const state = appState();
    if (runtime.session()?.mode === "daily") state.badgeMetrics.dailySessions += 1;
  ...
  runtime.wrapFunction("renderBadges", baseRenderBadges => function renderBadgesAsCatalogue() {
    syncEarned();
    baseRenderBadges.apply(this,arguments);
    renderCatalogue();
  });

  runtime.wrapFunction("switchView", baseSwitchView => function switchViewWithBadgeCelebrations(view) {
...
window[API] = Object.freeze({
  ...
  renderCatalogue,
  status() {
    return Object.freeze({installed:Boolean(window[INSTALL_FLAG]),celebrating,pending:runtime ? badgeState().pendingCelebrations.length : 0});
  }
});
install();
})();
