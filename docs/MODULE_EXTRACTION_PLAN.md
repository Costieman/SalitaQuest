...
23. `social-connections-v2.js` — adapter-led split into `src/adapters/sharing/social-connections-runtime-v1.js` and `src/features/sharing/social-connections-v2.js`; profile/API-override storage and navigation replacement now cross the runtime boundary, while the historical root remains the ordered compatibility coordinator through r77.
