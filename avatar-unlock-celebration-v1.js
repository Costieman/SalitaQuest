(() => {
  ...
  const INSTALL_FLAG = "__salitaQuestAvatarUnlockCelebrationInstalled";
  ...
  function nextPending(sourceCollection, model) {
    ...
    return collection.pendingUnlocks.find(entry => {
      ...
      return item && collection.ownedAvatarIds.includes(item.id) && entry.animationSeen !== true;
    }) || null;
  }
  ...
  function consumePending(sourceCollection, pendingEntry, model, now = new Date().toISOString()) {
    ...
    collection.pendingUnlocks = collection.pendingUnlocks.filter(entry => {
      if (consumed || entry.animationSeen === true) return entry.animationSeen !== true;
      const matches = entryKey(entry) === targetKey || (
        entry.avatarId === pendingEntry?.avatarId &&
        entry.source === pendingEntry?.source &&
        entry.animationSeen !== true
      );
      if (!matches) return true;
      consumed = {...entry, animationSeen:true, animationSeenAt:now};
      return false;
    });
    return Object.freeze({collection, consumed});
  }

  globalRoot.SalitaAvatarUnlockCelebrationLogic = Object.freeze({entryKey, nextPending, consumePending});
  ...
  function reducedMotion() {
    try {
      if (typeof state !== "undefined" && state.settings?.reducedMotion) return true;
    } catch {}
  ...
  function homeIsReady() {
    ...
    if (!home) return true;
    return home.classList.contains("active") || document.body.dataset.currentView === "home";
  }
  ...
  function playChime() {
    try {
      ...
      [659.25, 783.99, 987.77, 1318.51].forEach((frequency, index) => {
        ...
        oscillator.start(start + index * .10);
        oscillator.stop(start + index * .10 + .46);
      });
  ...
  function buildLayer(item, entry) {
    ...
    node.innerHTML = `
      ...
      <section class="sq-avatar-unlock-card">
        ...
        <p>${copy.text}</p>
        <span class="sq-avatar-unlock-meta">${item.rarity} · ${item.category}</span>
  ...
  function saveCompletion(pendingEntry, item) {
    ...
    if (result.consumed) {
      profile.avatarUnlockHistory.push({
        avatarId:item.id,
        source:result.consumed.source || "unknown",
        level:result.consumed.level || null,
        weekKey:result.consumed.weekKey || null,
        unlockedAt:result.consumed.unlockedAt || null,
        animationSeenAt:result.consumed.animationSeenAt
      });
  ...
  async function openCollectionTarget(item) {
    ...
    if (!card) {
      await wait(300);
      card = document.querySelector(`[data-avatar-card="${CSS.escape(item.id)}"]`);
    }
    if (card) {
  ...
  async function flyToCollection(item, pendingEntry) {
  ...
  function skipAnimation(item, pendingEntry) {
    layer?.remove();
    layer = null;
    saveCompletion(pendingEntry, item);
  }
  ...
  async function playNext() {
    ...
    if (playing || celebrationBlocked() || !model) {
    ...
    const finish = async useFlight => {
      if (!playing) return;
      playing = false;
      if (useFlight) await flyToCollection(item, pendingEntry);
      else skipAnimation(item, pendingEntry);
  ...
  function install() {
    ...
    document.addEventListener("click", () => { if (!playing) schedule(700); }, {passive:true});
    window.SalitaAvatarUnlockCelebration = Object.freeze({playNext, schedule});
  }
